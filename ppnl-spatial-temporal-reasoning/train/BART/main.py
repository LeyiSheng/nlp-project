...
def main() -> None:
	...
	if "wandb" in training_args.report_to and training_args.local_rank <= 0:
		...
		wandb.init(
			project=os.getenv("WANDB_PROJECT", "Smcalflow"),
	...
	# if isinstance(tokenizer, BartTokenizerFast):
		# In Bart `<` is OOV, see https://github.com/google-research/language/blob/master/language/nqg/tasks/spider/restore_oov.py
	...
	if data_args.dataset in ["spider", "spider_realistic", "spider_syn", "spider_dk"]:
			trainer = SpiderTrainer(**trainer_kwargs, callbacks = [EarlyStoppingCallback(30)])
	else:
